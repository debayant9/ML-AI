...
class TicTacToe():
    ...
    def is_winning(self, curr_state):
        ...
        Output = False"""
        curr_state = np.nan_to_num(curr_state, 0)
    ...
    def step(self, curr_state, curr_action):
        ...
        result = self.is_terminal(new_state)
        if result[1] == 'Win':
            return (new_state, 10, True)
        elif result[1] == 'Tie':
            return (new_state, 0, True)
        else:
            rew = self.turn_of_env(new_state)
        
        if rew[1] == -10:
            return (rew[0], rew[1], True)
        elif rew[1] == 0:
            return (rew[0], rew[1], True)
        else:
            return (rew[0], rew[1], False)
        
        
    def turn_of_env(self, curr_state):
        newstate_after_turnofenv = self.state_transition(curr_state, random.choice(self.action_space(curr_state)[1]))
        res = self.is_terminal(newstate_after_turnofenv)[1]
        if res == 'Win':
            return newstate_after_turnofenv, -10
        elif res == 'Tie':
            return newstate_after_turnofenv, 0
        else:
            return newstate_after_turnofenv, -1
